...
async function createGroup() {
    ...
    try{
        const res = await axios.post('http://44.193.6.13:3000/group/createGroup', { groupName: groupName }, { headers: { 'Authorization': token }})
    }
    catch(error) {
        console.log(error);
        if(error.response){
...
async function showGroups() {
    try{
        ...
        const res = await axios.get('http://44.193.6.13:3000/group', { headers: { 'Authorization': token }})
...
async function addGroupUsers(groupId) {
    ...
    try{
        const addUserRes = await axios.post(`http://44.193.6.13:3000/group/${groupId}/addUser`, {userEmail: userEmail}, { headers: { 'Authorization': token }});
...
async function getGroupUsers(group) {
    try {
        const res = await axios.get(`http://44.193.6.13:3000/group/${group.id}/users`, { headers: { 'Authorization': token }});
...
async function removeGroupUsers(groupId, userId) {
    try {
        const deleteRes = await axios.delete(`http://44.193.6.13:3000/group/${groupId}/user/${userId}`, { headers: { 'Authorization': token }});
