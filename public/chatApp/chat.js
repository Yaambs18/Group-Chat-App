...
async function sendMsg(category, categoryId) {
    ...
    try {
        ...
        if(file.files.length > 0){
            ...
            if(category==='group'){
                const result = await axios.post(`http://44.200.225.63:3000/chat/uploadFile/grpMsg/${categoryId}`, newFile, { headers: {"content-type": "multipart/form-data", 'Authorization': token }});
                displayMessage(result.data.message);
            }
            else if(category === 'user'){
                const result = await axios.post(`http://44.200.225.63:3000/chat/uploadFile/userMsg/${categoryId}`, newFile, { headers: {"content-type": "multipart/form-data", 'Authorization': token }});
                displayMessage(result.data.message);
            }
        }
        else{
            ...
            if(category==='group'){
                const result = await axios.post(`http://44.200.225.63:3000/chat/grpMsg/${categoryId}`, msgObj, { headers: { 'Authorization': token }});
                displayMessage(result.data.message);
            }
            else if(category === 'user'){
                const result = await axios.post(`http://44.200.225.63:3000/chat/userMsg/${categoryId}`, msgObj, { headers: {'Authorization': token }});
                displayMessage(result.data.message);
            }
            msgData.value = '';
        }
...
async function getMessages(category, categoryId) {
    try{
        ...
        const res = await axios.get(`http://44.200.225.63:3000/chat/messages?category=${category}&categoryId=${categoryId}&lastMsgId=${lastMsgId}`, { headers: { 'Authorization': token }});
